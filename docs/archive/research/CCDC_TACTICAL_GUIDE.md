> [!WARNING]
> **Historical research archive — not current operational guidance.**
> Recovered from dropped pre-release history on 2026-08-14. Its claims, URLs, tool behavior, and defensive recommendations have not been revalidated and may now be stale or unsafe. Use only on systems you are authorized to assess, and independently verify every claim before acting.
> References to Pandora's Box password rotation, password-on-command-line examples, broad platform support, or legacy commands describe removed pre-release behavior and are not current product claims.
...
# Removed: obsolete password-on-command-line example.
