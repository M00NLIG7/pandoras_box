...
fn artifact_root() -> PathBuf {
    std::env::var("PANDORAS_BOX_LIVE_WINDOWS_SSH_ARTIFACT_ROOT")
        .or_else(|_| std::env::var("PANDORAS_BOX_LIVE_ARTIFACT_ROOT"))
        .map(PathBuf::from)
        .unwrap_or_else(|_| temp_root("live-windows-ssh"))
}
...
#[tokio::test]
#[ignore = "requires a local Tiny11/OpenSSH target; run scripts/run-windows-ssh-interop.sh"]
async fn live_windows_ssh_target_collects_inventory_and_cleans_up() {
    ...
    let target_ip = required_env("PANDORAS_BOX_LIVE_WINDOWS_SSH_HOST")
        .parse::<IpAddr>()
        .expect("interop host should be a valid IP address");
    let ssh_port = required_env("PANDORAS_BOX_LIVE_WINDOWS_SSH_PORT")
        .parse::<u16>()
    ...
    let username = required_env("PANDORAS_BOX_LIVE_WINDOWS_SSH_USERNAME");
    let password = required_env("PANDORAS_BOX_LIVE_WINDOWS_SSH_PASSWORD");
    ...
    assert_eq!(
        ...
        "windows",
        "Tiny11 should classify as Windows when SSH and SMB are forwarded"
    );

    let summary = PandorasBoxRunner::new(spec)
        .run()
        .await
        .expect("live Windows SSH interop run should succeed");
    ...
    assert!(
        plan.contains("\"transport_chain\": [\"windows_ssh\", \"windows_smb\"]"),
        "plan should prefer Windows SSH before SMB fallback: {plan}"
    );
    ...
    if rotation_magic.is_some() {
        ...
        assert!(
            failed_status.to_ascii_lowercase().contains("authenticate"),
