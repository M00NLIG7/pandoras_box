...
// Define Winrm Client (subject behind)
pub struct WinrmClient {
    // session: Option<Arc<Mutex<Session>>>, // Winrm Session
...
impl WinrmClient {
    ...
    pub async fn connect(
        &mut self,
        creds: &Credentials,
    ) -> Result<WinrmSession, Box<dyn std::error::Error>> {
        if self.ip.is_none() {
    ...
    async fn is_container_running() -> Result<bool, std::io::Error> {
        let runc_path = if WinrmClient::is_runc_in_path()? {
            "runc"
        } else {
            "/tmp/runc"
        };
        ...
        Ok(output_str.contains("winrm-container"))
    }
    ...
    async fn establish_connection(
    ...
    ) -> Result<WinrmSession, std::io::Error> {
        ...
        let mut cmd = Command::new(if needs_container { runc_path } else { "winrm" });

        // Container specific arguments
        if needs_container {
            cmd.arg("exec").arg("winrm-container").arg("winexe");
        }
    ...
    async fn install_runc() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if WinrmClient::is_runc_in_path()? {
            return Ok(());
        }
